...
//        read.nextLine();
    ...
    private void undo(){
        if(undo.isEmpty()){
            System.out.println("CANNOT UNDO! Undo stack empty!");
        }

        else{
            ArrayList<Employee> newEmployees = new ArrayList<Employee>();
            newEmployees = copyEmployee(employees);
            redo.push(newEmployees);

            ArrayList<Employee> finalUndo = new ArrayList<Employee>();
            if(undo.size() == 1){
                finalUndo = copyEmployee(undo.peek());
            }
            else
                finalUndo = copyEmployee(undo.pop());
    ...
    private void redo(){
        if(redo.isEmpty()){
            ...
            ArrayList<Employee> finalRedo = new ArrayList<Employee>();
            finalRedo = copyEmployee(redo.pop());
            employees = finalRedo;
